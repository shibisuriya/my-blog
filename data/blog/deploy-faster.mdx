---
title: 'Deploy frontend projects in under 30 seconds!'
date: '2024-07-04'
lastmod: '2024-07-04'
tags: ['devops', 'jenkins', 'pipeline', 'CI/CD', 'Github actions', 'hack', 'deployment', 'productivity']
draft: false
...
here the frontend projects are structured into modular units called module federation
remotes and hosts - microfrontends. The deployment process is already efficient,
we only have to build and deploy the units in which changes were made, instead of building and deploying the entire project.
...
work to save some time, working in parallel, just like a computer! Sometimes I get caught up in the task, and forget to check the status of the build... Due to this silly
reason I have failed to provide relavant stakeholders with the updated build and kept them waiting multiple times :(

I have talked to multiple of my friends regarding this, and asked them for advice. Some of the good advices were,

- Adding a step in Jenkins which would notify me regarding the status of the build through gchat, discord, vs code etc.

- Taking a tea break after initiating a build instead of doing some work in parallel. Only doing one thing at a time!

Since I don't know anything about build pipelines I have been taking tea breaks, I don't have the habbit of drinking tea, so I pretty much sit ideally while waiting for the
build to complete.

I have been studying build pipelines and deployment stategies recently, and found that the 'steps' that the pipeline will follow is written inside a file
that is stored in the project's repository itself! By modifying this file you would be able to control the steps the pipeline will take!

I started to regularly monitor the build logs to find out steps that were redundant or inefficient, so that I could remove or improve them.
...
or, must better, request webpack's dev server to write to disk.
...
- Setting this up could be difficult depending on your system's architecture.
