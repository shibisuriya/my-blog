...
Let's assume our notes are located at `~/Desktop/office-notes/kissflow-feature-x` and our project's Git repository is at `~/Desktop/projects/kissflow-frontend`.

Then, we can symlink our notes directory to a directory inside the codebase.

```bash
cd ~/Desktop/projects/kissflow-frontend
mkdir temp-notes # First, create a directory that is ignored by git (using .gitignore_global).
ln -s ~/Desktop/office-notes/kissflow-feature-x ~/Desktop/projects/kissflow-frontend/temp-notes/
```
